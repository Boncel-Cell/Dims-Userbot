...
async def get_full_user(event):
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("`Terjadi Kesalahan... Mohon Lapor Ke Grup` @DimsUserbotSupport", str(err))
...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
# Ported For Dims-Userbot by Dims/Dimswhydi
...
async def gben(userbot):
    ...
    if not sender.id == me.id:
        dark = await dc.reply("`Anda Ingin Mengaktifkan Perintah Global Banned!`")
    else:
        dark = await dc.edit("`Memproses Global Banned Pengguna Ini`")
    me = await userbot.client.get_me()
    await dark.edit(f"`Global Banned Akan Segera Aktif, Anda Akan Dibanned Secara Global Oleh Dims`")
    ...
    if user:
        if user.id == 1353102497:
            return await dark.edit(
                f"`Anda Tidak Bisa Melakukan Global Banned Ke Dims Ganteng, Dia Adalah Pembuat Saya ヅ`"
            )
        try:
        ...
        await dark.edit(f"`Mohon Balas Ke Pesan Tolol`")
    try:
        ...
        pass
    return await dark.edit(
        f"**Perintah Babu Dims:** `{ALIVE_NAME}`\n**✣ Pengguna:** [{user.first_name}](tg://user?id={user.id})\n**✣ Aksi:** `Global Banned`"
    )
...
async def gunben(userbot):
    ...
    me = await userbot.client.get_me()
    await dark.edit(f"`Memulai Membatalkan Perintah Global Banned, Pengguna Ini Akan Dapat Bergabung Ke Grup Andaヅ`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await userbot.get_chat()
    a = b = 0
    ...
    if user:
        ...
        await dark.edit("`Harap Balas Ke Pesan Pengguna Tololヅ`")
    try:
